...
int main(int argc, char* argv[]) {
    ...
    // if(true){
    //     maskRCNN = std::make_unique<MaskRCNN>(nullptr); // queue in
    //     sourceCode
    //     // sequentialMaskRCNN = (queue == nullptr);
    // }

    // if (maskFusion->processFrame(logReader->getFrameData(), currentPose,
    // weightMultiplier) && !showcaseMode) {
    ...
    // SegmentationResult MaskFusion::performSegmentation(FrameDataPointer
    // frame) {
    //     return labelGenerator.performSegmentation(models, frame,
    //     getNextModelID(), spawnOffset >= modelSpawnOffset);
    // }
...
// #endif
    ...
    // #ifdef WAYNE_DEBUG
    //     std::cout<<"\nPre execute Sequential\n";
    // #endif

    // if(!maskRCNN) throw std::runtime_error("MaskRCNN is not embedded and no
    // masks were pre-computed."); else if(sequentialMaskRCNN)
    // maskRCNN->executeSequential(frame);
    // }

#ifdef WAYNE_DEBUG
    std::cout << "\nProgram passed MaskRCNN.\n";
#endif

    // #ifdef WAYNE_DEBUG
    //     cv::imwrite(WRITE_MASK_DIR + "mrcnn" + std::to_string(frame->index) +
    //     ".png", frame->mask);
    // #endif

#ifdef WAYNE_DEBUG
    std::cout << "\nProgram passed Image save.\n";
#endif
    ...
    // baseDir + depthDir, baseDir + maskDir,
    //                                                 Parse::get().arg(argc,
    //                                                 argv, "-indexW", indexW)
    //                                                 > -1 ? indexW : 4,
    //                                                 colorPrefix, depthPrefix,
    //                                                 maskPrefix,
    //                                                 Parse::get().arg(argc,
    //                                                 argv, "-f", tmpString) >
    //                                                 -1);
...
//   std::string depthImagePath = depthImagesDir + depthPre + indexStr +
//   depthExt; if (!boost::filesystem::exists(depthImagePath)) throw
//   std::invalid_argument("Could not find depth-image file: " +
//   depthImagePath);

//   std::string rgbImagePath = file + colorPre + indexStr + colorExt;
//   if (!boost::filesystem::exists(rgbImagePath)) throw
//   std::invalid_argument("Could not find rgb-image file: " + rgbImagePath);

//   // Load mask ids
//   //std::string maskImagePath = maskImagesDir + maskPre +
//   std::to_string(index); std::string maskImagePath = maskImagesDir + maskPre
//   + indexStr; std::string maskDescrPath = maskImagePath + ".txt";
//   maskImagePath += maskExt;
//   if (hasMasksGT) {
//     if (!boost::filesystem::exists(maskImagePath)) throw
//     std::invalid_argument("Could not find mask-image file: " +
//     maskImagePath); if (boost::filesystem::exists(maskDescrPath))
//     loadMaskIDs(maskDescrPath, &result->classIDs, &result->rois);
//   }

//   // Load RGB
//   result->rgb = cv::imread(rgbImagePath);
//   if (result->rgb.total() == 0) throw std::invalid_argument("Could not read
//   rgb-image file."); result->flipColors();

//   // Load Depth
//   result->depth = cv::imread(depthImagePath, cv::IMREAD_UNCHANGED);
//   if (result->depth.total() == 0) throw std::invalid_argument("Could not read
//   depth-image file. (Empty)"); if (result->depth.type() != CV_32FC1) {
...
//         ((float*)newDepth.data)[depthIdx++] = pixel[j][0];
//       }

//     } else if (result->depth.type() == CV_16UC1) {
//       //std::cout << "Warning -- your depth scale is likely to mismatch.
//       Check ImageLogReader.cpp!" << std::endl; unsigned depthIdx = 0; for
//       (int i = 0; i < result->depth.rows; ++i) {
//         unsigned short* pixel = result->depth.ptr<unsigned short>(i);
//         for (int j = 0; j < result->depth.cols; ++j)
//         ((float*)newDepth.data)[depthIdx++] = 0.001f * pixel[j];
//         //for (int j = 0; j < result->depth.cols; ++j)
//         ((float*)newDepth.data)[depthIdx++] = 0.0002f * pixel[j]; // FIXME
//       }
//     } else {
//       throw std::invalid_argument("Unsupported depth-files: " +
//       cvTypeToString(result->depth.type()));
//     }
...
//     if (result->mask.total() != result->rgb.total()) throw
//     std::invalid_argument("Could not read mask-image file."); if
//     (!result->mask.isContinuous() || result->mask.type() != CV_8UC1) throw
//     std::invalid_argument("Incompatible mask image.");
//   }
...
//   for (unsigned i = 0; i < 15 && nextBufferIndex < frames.size(); ++i,
//   ++nextBufferIndex) {
...
//     bool MaskFusion::processFrame(FrameDataPointer frame, const
//     Eigen::Matrix4f* inPose, const float weightMultiplier, const bool
//     bootstrap) {
...
//     GL_LUMINANCE, GL_FLOAT); filterDepth();

//     // if(frame.mask) {
//     //    // Use ground-truth segmentation if provided (TODO: Overwritten at
//     the moment)
//     //    textureMask->texture->Upload(frame.mask, GL_LUMINANCE_INTEGER_EXT,
//     GL_UNSIGNED_BYTE);
//     //} else
//     if (!enableMultipleModels) {
//         // If the support for multiple objects is deactivated, segment
//         everything as background (static scene). const long size =
//         Resolution::getInstance().width() *
//         Resolution::getInstance().height(); unsigned char* data = new
//         unsigned char[size]; memset(data, 0, size);
//         textureMask->texture->Upload(data, GL_LUMINANCE_INTEGER_EXT,
//         GL_UNSIGNED_BYTE); delete[] data;
//     }
...
//     if(frame->mask.total() == 0) {
//         if(!maskRCNN) throw std::runtime_error("MaskRCNN is not embedded and
//         no masks were pre-computed."); else if(sequentialMaskRCNN)
//         maskRCNN->executeSequential(frame);
//     }
